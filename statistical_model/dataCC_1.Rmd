...
# Compute per-shoe contact pixel counts for offset calculation
shoe_ids <- unique(acciden$shoe)
pixel_counts <- data.frame(
  shoe = shoe_ids,
  pixel_count = sapply(shoe_ids, function(i) sum(data_temp[[i]]))
)
write.csv(pixel_counts, paste(ROOT_PATH, "processed_data/shoe_pixel_counts.csv", sep = ""), row.names = FALSE)
cat("Mean pixel count:", mean(pixel_counts$pixel_count), "\n")
cat("Mean offset log(N_CONTROLS/pixels):", mean(log(N_CONTROLS / pixel_counts$pixel_count)), "\n")
...
#which includes all cases (pixels with RACs, nij = 1) and N_CONTROLS random controls (pixels without RACs, nij = 0) from each shoe
dataCC <- numeric()
for(i in 1:length(unique(shoe)))
{
  case <- mydata[mydata$shoe==i&mydata$n_Acc>0,]
  control <- mydata[mydata$shoe==i&mydata$n_Acc==0,]
  control <- control[sample(nrow(control), size = N_CONTROLS, replace = FALSE),]
