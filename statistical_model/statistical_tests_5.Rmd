...
avg <- newdesignmat %*% fixef(random_model) + MEAN_OFFSET
...
```{r linearHypothesis_test}
# ============================================================
# Hypothesis test: all spatial coefficients = 0
# Ported from naomikap/rac-intensity, Code/pixel analysis.R ~line 370
# Tests H0: beta_1 = beta_2 = ... = beta_p = 0 (all except intercept)
# Uses the random effects (glmer) estimator
# ============================================================

co <- fixef(random_model)
vc <- vcov(random_model)

# Contrast matrix: identity matrix minus first row (intercept)
matr <- diag(length(co))[-1, ]

testing <- linearHypothesis(random_model,
                            hypothesis.matrix = matr,
                            rhs = rep(0, length(co) - 1),
                            test = c("Chisq", "F"),
                            vcov. = vc, coef. = co)
print(testing)
```
